...
def write_code_file():
    speech_output.speak("what should we call this code file?")
    filename = voice_input.capture_voice()
    with open(f"{filename}.py", "a") as file:
        print(f"File {filename}.py written successfully!")
        speech_output.speak(f"File {filename}.py written successfully!")
    while True:
        write_code_file_from_voice(filename)
        

def write_code_file_from_voice(filename):
    print("Listening for code input...")
    ...
    if voice_text:
        ...
        print(f"Code written to {filename}.py successfully!")
        print(f"Your code:\n{interpreted_code}")
